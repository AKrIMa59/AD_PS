...
# Obtenir tous les comptes utilisateurs inactifs depuis la date limite
$comptesInactifs = Get-ADUser -Filter {Enabled -eq $true -and LastLogonDate -lt $dateLimite} -Properties mail, SamAccountName, LastLogonDate, lastLogonTimestamp

Add-Content $FilePath "SamAccountName;LastLogonDate;mail;lastLogonTimestamp"
...
foreach ($compte in $comptesInactifs) {
    ...
    Add-Content $FilePath "$($compte.SamAccountName);$($compte.LastLogonDate);$($compte.mail);$($lastLogonTimestampFormatted)" 
}
